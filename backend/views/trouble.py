...
def trouble_create(request):
    if request.method == 'GET':
        ...
        if form.is_valid():
            ...
            dic['status'] = 1
            # 在dic中加入from中的字段資料(title,detail)
...
def trouble_json_report(request):
    ...
    for user in user_list:
        ...
        # 執行原生sql
        cursor.execute(
            """select strftime('%%s',strftime("%%Y-%%m-01",ctime)) * 1000,count(id) from repository_trouble where processer_id = %s group by strftime("%%Y-%%m",ctime)""",
            [user.nid, ])
